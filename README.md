...
The infrastructure fundamentally operates on an asymmetric distributed topology designed to minimize dependencies on centralized cloud services, with the objective of reducing cloud inference OpEx to near-zero. The system boundary is divided into two distinct executable layers:

### 1. neurodoc-ai-client (Universal Edge Engine)
The client layer (`ai-governance`) is a lightweight, statically compiled binary engineered in Rust and native C. It implements an explicit **Dual-Role Execution Model** toggled via local configuration flags:
*   **Consumer Mode (Standard Pipeline Interceptor)**: Operates seamlessly inside pre-flight CI/CD or local git hook loops. It reads the raw AST token stream from standard input (`STDIN`), extracts structured repository metadata, packages the manifest, and routes the encrypted transaction payload to the central sovereign orchestrator.
*   **Worker Mode (Federated Silicon Allocation)**: Transforms the local developer machine into an active grid computing node. The native Rust/C inference subsystem operates primarily through unmanaged off-heap memory, executing local inference pipelines with minimal runtime overhead. Inspired by the open-source hardware-software co-design of advanced asynchronous compiler stacks (specifically the MLIR-based graph pipeline in Tenstorrent's [tt-forge](https://github.com/tenstorrent/tt-forge) architecture), the platform aims to avoid the overhead of global hardware schedulers. Instead, the proposed runtime constructs and balances the semantic execution pipeline inside the local code compiler before inference begins. It fetches precise context blocks directly into isolated memory slots, aiming to mitigate token bottlenecks and enabling resource-constrained servers to behave as a unified local inference fabric while keeping cloud inference OpEx effectively near zero.

*   **Hardware-Aware Execution Orchestration (Silicon Telemetry)**: Before executing any local inference workload, the runtime evaluates the capabilities of the host workstation through a telemetry-driven scheduling layer. Available system memory, CPU resources, GPU/VRAM capacity, model requirements, context size, and execution constraints are combined into an execution suitability assessment that determines whether the workload should remain on the local machine or be transparently delegated to another execution backend. The proposed telemetry subsystem can leverage open-source hardware profilers (e.g. [llmfit](https://github.com/AlexsJones/llmfit)) to estimate execution suitability across heterogeneous compute environments. When local resources are insufficient, the client forwards only the minimum semantic context required to the orchestration layer, allowing execution to be delegated to another federated worker, privately managed rented infrastructure running open-source models, or—when explicitly configured—a commercial frontier AI provider. This hardware-aware scheduling preserves a unified execution model while maximizing local execution, avoiding Out-Of-Memory (OOM) failures, and minimizing unnecessary cloud inference costs. The resulting execution decision directly determines whether the proposed Context-Commit Memory Topology can be applied locally or whether only the minimal semantic context should be propagated to a remote execution backend.

*   **Proposed Context-Commit Memory Topology (Off-Heap RAM Alleviation)**: To explore alternatives to the industry trend of increasingly large VRAM requirements, the architectural roadmap proposes decoupling the expanding context footprint from GPU memory. Inspired by recent research on asynchronous inference fabrics (such as Kimi Delta Attention), the design explores treating GPU VRAM as transient compute memory while persisting accumulated semantic context ("context commits") inside unmanaged off-heap host RAM buffers. During inference, only the active semantic delta would be transferred across the PCIe bus, with the objective of allowing commodity workstations (e.g., 16 GB VRAM GPUs paired with 32 GB system RAM) to efficiently execute long-context workloads while minimizing cloud inference costs.

### 2. neurodoc-ai-server (Sovereign Multi-Tenant Rector)
The sovereign orchestration core of the ecosystem is implemented using Java 25 and Spring Boot 4.1.0. It serves as the Single Source of Truth, handling strict cross-domain Vertical Slicing isolation rules, managing cold relational persistence via PostgreSQL / pgvector, and maintaining semantic workspace embeddings within localized ChromaDB instances. 

The architecture acts as a central distributed compute broker that orchestrates query load-balancing across all available federated Client Workers on the mesh. While the system operates 100% on local hardware by default, the orchestration layer maintains native adapters capable of provisioning self-managed inference runtimes on rented bare-metal or GPU infrastructure and seamlessly offloading high-concurrency inference workloads whenever additional compute capacity is required. This enables organizations to deploy their own quantized open-source models (such as Qwen-72B or Llama-70B) onto leased compute resources under private runtime isolation, rather than relying on proprietary cloud inference APIs. By separating infrastructure rental from AI service consumption, the platform preserves code and data sovereignty, minimizes cloud inference OpEx, and eliminates third-party telemetry during self-hosted execution.
...
* **Hermetic Runtime Security**: Native mitigation of the "processing-in-use" vulnerability via non-GC off-heap memory segments, entirely bypassing the multi-tenant data leakage risks found in traditional multi-node Kubernetes topologies.
...
- [ ] **Target Demo 01: The Out-of-Memory Storage Engine Saturation (The 20-Cycle Task Test)**
  ...
  * **The Chaos:** Trigger concurrent lookups and contract audits simultaneously from 64 autonomous agents, forcing massive, unbuffered random read operations on the persistent secondary storage.
    - **The Hexagonal Recovery:** Instead of crashing the cluster with an Operating System Out-Of-Memory (OOM) fault or slowing down due to kernel thread context switching, the core domain switches dynamically to our low-overhead storage engine parameters. It bypasses the Linux page cache via `O_DIRECT`, forces **atomic 4 KB page granularities** to reduce I/O amplification, and leverages a **high-efficiency Context Cache Hit model**. This ensures that multiple agents working concurrently over the same master document repository reuse the hot context memory instantly without consuming extra computing cycles or triggering redundant disk reads. Finally, thread management is dropped down to **user-space cooperative multitasking** (using Rust async executors).
